...
def main():
	...
	stream_df = stream_df.groupBy('stream_id', 'gen_name').agg(functions.max('viewers').alias('viewers'))
	stream_df = stream_df.groupBy('gen_name').agg(functions.sum('viewers').alias('total_viewers'))
	stream_df = stream_df.where(stream_df['gen_name'].isin("fortnite", 
		"call of duty®: black ops 4", 
		"league of legends", "overwatch", 
		"counter-strike: global offensive", 
		"playerunknown's battlegrounds", "just chatting", 
		"fallout 76", "tom clancy's rainbow six: siege", 
		"world of warcraft", "red dead redemption 2", 
		"destiny 2", "dota 2", "fifa 19", "battlefield v", 
		"dead by daylight", "rocket league", "grand theft auto v", "nba 2k19", "minecraft"))
	
	stream_df = stream_df.orderBy(stream_df.total_viewers.desc())
	sdf = stream_df.toJSON().collect()
	with open('top_20_game_views', 'w') as output:
		json.dump(sdf, output)
		output.close()
	#json.dump(json.JSONDecoder().decode(write), 'testdump', indent = 4, separators = (', ', ': '))
